...
public class CordovaActivity extends Activity implements CordovaInterface {
    ...
    */
    ...
    protected Dialog getSplashDialog(CordovaActivity cordovaActivity) {
        Dialog dialog;
        try {
            dialog = new Dialog(cordovaActivity, getPackageManager().getActivityInfo(getComponentName(), 0).theme);
        }
        catch (PackageManager.NameNotFoundException e) {
            Log.d(TAG, "Unable to get the package's theme");
            dialog = new Dialog(cordovaActivity, android.R.style.Theme_Translucent_NoTitleBar);
        }
        return dialog;
    }
    ...
    protected void showSplashScreen(final int time) {
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                ...
                splashDialog = getSplashDialog(that);
